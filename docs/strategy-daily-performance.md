...
- ~~**Investigate `tracker.py`'s `pnl_delta` computation on the single-close path**~~ — **fixed 2026-09-25** in `signal_engine/tracker.py`'s `_book_broker_close()`. In ANALYZE mode, the single-close path now derives `pnl_delta` from price (`ltp` vs. the position's own fill/entry price) instead of trusting the sandbox's "realised" figure, the same way the partial-exit path already did (2026-09-21 fix). LIVE mode is unchanged — a real broker's `ltp` is the last traded market price, not a guaranteed fill price, so its own realised figure remains the trusted source there. Regression tests: `signal_engine/tests/test_close_accounting.py::TestSandboxSingleCloseTrustsPriceNotTheBrokerFigure`. See `signal_engine/PRD.md`'s 2026-09-25 entry for the fix detail.
